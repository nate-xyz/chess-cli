...
def play_stockfish( board_window, move_str, board, status_str, \
                    entered_move_bool, history_arr, \
                    game_outcome_enum, move_amount, final_position_str, \
    ...
    while (key != 15): #while not quitting
        if player_control == True:
            ...
            move_str = stockfish.get_best_move_time(1000) 
    ...
    return (move_str, board, status_str, entered_move_bool, last_move_str, \
            history_arr, game_outcome_enum, move_amount, final_position_str,\
...
def draw_board(board_window, board_FEN, board_square_coord, pieces):
    ...
    for i in range(len(board_FEN)): #loop to parse the FEN stirng
        ...
        if current_piece == '/':
            ...
            pieces[current_piece.upper()]+" ")
            
            board_square_coord[key_tuple] = (is_floating_bool_color, \
            pieces[current_piece.upper()])
